            ...
            <div class="kt-portlet__head-toolbar">
                <div class="kt-portlet__head-wrapper">
                    <div class="kt-portlet__head-actions">&nbsp;
                        @can('partners-store')
                            <a onclick="openModal()" class="btn text-white btn-brand btn-elevate btn-icon-sm" style="color: #fff ; background-color: #f90000 ;border: unset">
...
@push('js')
    ...
    <script>
        ...
        table.DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin.successes.data') }}',
            columns: [
                {data: 'pk_i_id'},
                {data: 's_title', name: 'translations.s_title'},
                {data: 'dt_created_date'},
                {data: 'dt_modified_date'},
                {data: 'enabled_html', searchable: false, sortable: false, visible: {{ (int) auth()->user()->can('partners-status') }}},
                {
                    data: 'actions_column',
                    searchable: false,
                    sortable: false,
                    responsivePriority: -1,
                    visible: {{ (int) (auth()->user()->can('partners-store') || auth()->user()->can('partners-delete')) }}
                },
            ]
        });
        ...
        function openModal(id = '') {
            $.get('{{ route('admin.successes.form') }}?id=' + id, function (data) {
